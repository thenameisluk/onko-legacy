...
dsl::sprite::sprite(uint32_t width,uint32_t height){
    this->width = width;
    this->height = height;
...
dsl::sprite::sprite(dataArray& data){
    PbView view(data);
...
dsl::sprite::sprite(sprite& other){
...
void dsl::sprite::resize(uint32_t width, uint32_t height){
    this->width = width;
    this->height = height;
    delete[] data;
    data = new uint8_t[width*height];
...
uint32_t dsl::sprite::getWidth(){
    return width;
};
uint32_t dsl::sprite::getHeight(){
    return height;
};
dsl::argb8888 dsl::sprite::getColor(uint8_t id){
    if(!id||id>15){
        return 0;
    }
    return pallet[id-1].color;
};
void dsl::sprite::setColor(uint8_t id,argb8888 color){
    if(!id||id>15){
        return;
    }
    pallet[id-1] = color;
};
uint8_t dsl::sprite::getPixelColorId(uint32_t x,uint32_t y){
    if(x<0||x>=width||y<0||y>=height){
        return 0;
    }
    return data[x+y*width];
};
dsl::argb8888 dsl::sprite::getPixelColor(uint32_t x,uint32_t y){
    return getColor(getPixelColorId(x,y));
};
void dsl::sprite::draw(int32_t x,int32_t y,uint8_t colorID){
    if(x<0||x>=width||y<0||y>=height){
        return;
    }
    data[x+y*width] = colorID;
};
dsl::dataArray dsl::sprite::expot(bool rle){//to-do rle
    PbWriter writer;
...
template <>
void dsl::ctxTemplate<dsl::argb8888>::drawSprite(int32_t x, int32_t y, sprite& spr, uint32_t scale,bool leftRightFlip,bool upDownFlip){//to-do flip
...
template <>
void dsl::ctxTemplate<dsl::argb8888>::drawSprite(vector2D pos,sprite& spr, uint32_t scale,bool leftRightFlip,bool upDownFlip){
    drawSprite(pos.x,pos.y,spr,scale,leftRightFlip,upDownFlip);
};
template <>
void dsl::ctxTemplate<dsl::argb8888>::drawSprite(int32_t destX,int32_t destY,int32_t formX,int32_t formY,uint32_t width,uint32_t height,sprite& spr,uint32_t scale,bool leftRightFlip,bool upDownFlip){
    for(int32_t ix = 0; ix < width; ix++){
        for(int32_t iy = 0; iy < height; iy++){
            uint8_t id = spr.getPixelColorId(ix+formX, iy+formY);
            if(id){
...
template <>
void dsl::ctxTemplate<dsl::argb8888>::drawSprite(vector2D pos,vector2D from,vector2D size,sprite& spr, uint32_t scale,bool leftRightFlip,bool upDownFlip){
